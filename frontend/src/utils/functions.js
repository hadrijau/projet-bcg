...
const addFareBecauseOfMiles = (distance) => {
    return (distance / 0.2) * 0.5
}

const isTimeBetween8And6 = (startTime) => {
    let hour = new Date(startTime).getHours();
    return hour >= 20 || hour <= 6;
}

const isBusyPeriod = (startTime) => {
    let hour = new Date(startTime).getHours();
...
export const computeEndTimeOfRide = (startTime, duration) => {
    ...
    let endTime = convertedTimeToDate.setSeconds((convertedTimeToDate.getSeconds()) + duration)
